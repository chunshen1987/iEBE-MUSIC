...
def analyze_Rspin_y(hf_, eventList_, outputFolder_: str,
                    pTmin_: float, pTmax_: float) -> None :
    """
...
def analyze_Rspin_pT(hf_, eventList_, outputFolder_: str,
                     rapMin_: float, rapMax_: float) -> None :
    """
        This function computes the event averaged R_spin(pT, y)
    """
    filelist = ["Rspin_pTy_{}_{}_3122.dat",
    ...
    for ipT in range(len(pTarr)):
        ...
        for icol in range(len(filelist)):
            f.write("%.5e  %.5e  " % (R_spin_avg[icol][ipT],
                                      R_spin_err[icol][ipT]))
        f.write("\n")
    f.close()
