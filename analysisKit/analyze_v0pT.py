...
def calculate_v0pT(pTArr, poiSP, refSP, pTmin: float, pTmax: float,
    ...
    for iev in range(nev):
        refSPInterp = np.exp(
            np.interp(pTInterp, pTArr, np.log(refSP[iev, :] + 1e-30)))
    ...
    for iev in range(nev):
        ...
        deltahat_poi_SP = (
            delta_poi_SP[array_idx, :]
            - np.mean(delta_poi_SP[array_idx, :]*deltaNref_local, axis=0)
    ...
    v0pT_err = np.sqrt((nev - 1)/nev*np.sum((v0pT_arr - v0pT_mean)**2., axis=0))
