...
for icen in range(len(centralityCutList) - 1):
    ...
    for event_name in selected_events_list:
        QnArr1.append(data[event_name]['STAR_eta_-1_-0p5'])
        QnArr2.append(data[event_name]['STAR_eta_0p5_1'])
        QnArr3.append(data[event_name]['STAR_eta_-0p5_0p5'])
    QnArr1 = np.array(QnArr1)
    QnArr2 = np.array(QnArr2)
    QnArr3 = np.array(QnArr3)
    calculate_pTfluct(QnArr1, QnArr2, QnArr3, "STAR", cenLabel)
