...
def calculateNonLinearResponseV3_2sub(vn_data_array1, vn_data_array2,
    ...
    chi_312_num = V3_1*(np.conj(V1_2*V2_2)) + V3_2*(np.conj(V1_1*V2_1))
    V1V2_V1V2 = 2*np.real((V1_1*V2_1*np.conj(V1_1*V2_2)))

    chi_312_JK = np.zeros(nev, dtype=complex)
    for iev in range(nev):
        array_idx = [True]*nev
        array_idx[iev] = False
        array_idx = np.array(array_idx)

        num_JK1 = np.mean(chi_312_num[array_idx])
        den_JK11 = np.mean(V1V2_V1V2[array_idx])

        chi_312_JK[iev] = num_JK1/den_JK11

    # compute the real part of the response coefficients
    chi_312_mean = np.mean(np.real(chi_312_JK))
    chi_312_err = np.sqrt((nev - 1.)/nev*np.sum(
        (np.real(chi_312_JK) - chi_312_mean)**2))
    results = [chi_312_mean, chi_312_err]

    # compute sqrt<V4L^2>
    V3L_1 = V3_1 - chi_312_mean*(V1_1*V2_1)
    V3L_2 = V3_2 - chi_312_mean*(V1_2*V2_2)
    v3L_rms = np.sqrt(np.mean(np.real(V3L_1*np.conj(V3L_2))))
    v3L_err = np.std(np.real(V3L_1*np.conj(V3L_2)))/np.sqrt(nev)/(2*v3L_rms)
    results += [v3L_rms, v3L_err]

    # compute the imaginary part of the response coefficients
    chi_312_mean = np.mean(np.imag(chi_312_JK))
    chi_312_err = np.sqrt((nev - 1.)/nev*np.sum(
        (np.imag(chi_312_JK) - chi_312_mean)**2))
    results += [chi_312_mean, chi_312_err]

    dN_mean = np.real(np.mean(vn_data_array1[:, 0] + vn_data_array2[:, 0]))
    dN_err = (np.std(vn_data_array1[:, 0] + vn_data_array2[:, 0])/np.sqrt(nev))
    if path.isfile(outputFileNameV3):
        f = open(outputFileNameV3, 'a')
    else:
        f = open(outputFileNameV3, 'w')
        f.write("# cen  Nch  Re{chi_312}  v3L_rms  Im{chi_312}\n")
...
for icen in range(len(centralityCutList) - 1):
    ...
    calculateNonLinearResponseV3_2sub(QnArr2, QnArr3,
                                      "nonLinearV3_2sub_full.dat", cenLabel)
